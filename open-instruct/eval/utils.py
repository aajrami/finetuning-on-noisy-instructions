...
from dispatch_openai_requests import dispatch_openai_chat_requesets, dispatch_openai_prompt_requesets
...
def replace_words_using_bert(text, model=None, tokenizer=None, percentage=0.25, random_seed=42):
...
def insert_words_using_bert(text, model=None, tokenizer=None, percentage=0.25, random_seed=42):
...
def misspell_words(word, random_seed=42):
    random.seed(random_seed)
    if len(word) < 3:
...
def introduce_typos(text, percentage=0.25, random_seed=42):
    random.seed(random_seed)
...
def delete_words(text, percentage=0.25, random_seed=42):
    random.seed(random_seed)
    # Split the text into words
    words = word_tokenize(text)
...
def shuffle_words(text, percentage=0.25, random_seed=42):
    random.seed(random_seed)
    # Split the text into words
    words = word_tokenize(text)
...
def remove_stop_words(text, percentage=1.0, random_seed=42):
